...
export async function GET(req: NextRequest) {
  ...
  if (!section || !prompts[section]) {
    return NextResponse.json({ content: null, error: "Sezione non valida" }, { status: 400 });
  }

  // Verifica API key
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error("[guida] ANTHROPIC_API_KEY non configurata");
    return NextResponse.json({ content: null, error: "ANTHROPIC_API_KEY mancante" }, { status: 500 });
  }
  ...
  if (propertyId) {
    try {
      ...
      console.error("[guida] Errore lettura property:", err);
      // non bloccante, si procede con il fallback
    }
  }
  ...
  try {
    ...
    console.error(`[guida] Errore Anthropic (section=${section}):`, msg);
    return NextResponse.json({ content: null, error: msg }, { status: 500 });
  }
}
